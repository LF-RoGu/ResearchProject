...
static ofstream csvRadar("_outFiles/radar_testReflectiveness9.csv");
static ofstream csvImu  ("_outFiles/imu_testReflectiveness9.csv");
#endif
            ...
            for (const TLVPayloadData& pd : frame.getTLVPayloadData())
            {
                ...
                for (size_t i = 0UL; i < pd.DetectedPoints_str.size(); ++i)
                {
                    ...
                    const bool is_valid =
                        ...
                        &&                                          
                        (closest_power > 2400U));                  // Minimum peak power:  
