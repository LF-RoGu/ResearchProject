...
static ofstream csvRadar("_outFiles/radar_testReflectiveness8.csv");
static ofstream csvImu  ("_outFiles/imu_testReflectiveness8.csv");
#endif
            ...
            for (const TLVPayloadData& pd : frame.getTLVPayloadData())
            {
                ...
                for (size_t i = 0UL; i < pd.DetectedPoints_str.size(); ++i)
                {
                    ...
                    const bool is_valid =
                        ...
                        &&                                          
                        (closest_power > 2350U));                  // Minimum peak power:  
