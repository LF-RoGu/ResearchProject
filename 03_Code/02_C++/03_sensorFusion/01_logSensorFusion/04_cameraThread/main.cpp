...
/*
Macro to enable the OpenCV webcam recorder
 0 - disable
 1 - enable
*/
...
const string fileSuffix = "labHallway1";  // << Change this only once
#if ENABLE_VIDEO
...
#if ENABLE_VIDEO
void threadVideoKeyWatcher(void)
{
    ...
    for (;;)
    {
        ...
        const int r = select(STDIN_FILENO + 1, &set, nullptr, nullptr, &tv);
        if (r > 0)
        {
            char ch;
            ...
            if ((n > 0) && (ch == 'q'))
            {
                std::cout << "[VIDEO] 'q' pressed. Stopping video." << std::endl;
                videoStopSignal.store(true);
                break;
            }
        }
    }
    ...
    std::cout << "[VIDEO] Key watcher thread exiting." << std::endl;
}
...
int main(void)
{
    ...
    #endif
    #if ENABLE_VIDEO
    thread thread_video_recorder(threadWebcamRecorder);
    thread thread_video_key(threadVideoKeyWatcher);
    #endif
    ...
    #endif
    #if ENABLE_VIDEO
    videoStopSignal.store(true); /* Ensure shutdown if main exits */
    if (thread_video_recorder.joinable()) { thread_video_recorder.join(); }
    if (thread_video_key.joinable())      { thread_video_key.join(); }
    #endif
