...
\begin{itemize}
    ...
    A single chirp spanning the full 4~GHz available bandwidth yields very fine range resolution, since $\Delta R$ decreases with larger $B$.  
    However, such steep slopes increase hardware demands on the ADC and limit the maximum measurable range $R_{max}$.  
    In practice, using the full bandwidth provides excellent spatial detail but may be less robust in longer-range detection scenarios.  

    \item \textbf{Multiple shorter chirps within the same frame} (e.g., two chirps each of 2~GHz instead of one 4~GHz chirp):  
    This allows the radar to trade off between coverage and resolution.  
    Each shorter chirp has reduced range resolution compared to the full 4~GHz sweep, but combining them can increase robustness and reduce ambiguity.  
    Multiple chirps with different slopes can also provide diversity in the frequency domain, which helps mitigate range-Doppler coupling and improves target detection in complex environments.  

    \item \textbf{Shorter chirps with smaller bandwidth}:  
    These increase maximum range but worsen range resolution.  
    For odometry, this means reliable detection of far objects but less precise localization of nearby structures, which are often most important for motion estimation.  

    \item \textbf{Chirp repetition interval}:  
    A short interval (fast chirp repetition) increases maximum unambiguous velocity, allowing the radar to track fast-moving targets.  
    Conversely, long intervals improve Doppler resolution but reduce $v_{max}$.  
    Thus, choosing the repetition rate is a balance between detecting high-speed objects and estimating fine velocity details.  
\end{itemize}
